...
<xsl:param    name="dateReport" select="''"/>
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			...
			<xsl:if test="$failureCount > 0">
				...
				<xsl:for-each select="/testResults/*[@lb = current()/@lb][attribute::s='false']">
					  <tr><th>Request Data</th><td><pre><xsl:value-of select="samplerData"/></pre></td></tr>
				      <tr><th>Response Data</th><td><pre><xsl:value-of select="responseData"/></pre></td></tr>
					  <tr><th>Response Code</th><td><pre><xsl:value-of select="@rc"/></pre></td></tr>
					  <tr><th>Response Message</th><td><pre><xsl:value-of select="@rm"/></pre></td></tr>
				      <tr><th>Failure Message</th><td><pre><xsl:value-of select="assertionResult/failureMessage"/></pre></td></tr>					  
				</xsl:for-each>
